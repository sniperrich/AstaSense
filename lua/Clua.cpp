...
namespace ns_menu
{
	...
	std::string add_check_box(sol::this_state s, std::string tab, std::string container, std::string label, std::string key, std::optional<bool> def, std::optional<sol::function> cb) {
		...
		//std::find(c_lua::get().menu_items[tab][container].begin(), c_lua::get().menu_items[tab][container].end(), item) != c_lua::get().menu_items[tab][container].end()
		return key;
	}
	...
	int add_slider_int(sol::this_state s, std::string tab, std::string container, std::string label, std::string key, int min, int max, std::optional<std::string> format, std::optional<int> def, std::optional<sol::function> cb) {
		std::transform(tab.begin(), tab.end(), tab.begin(), ::tolower);
		std::transform(container.begin(), container.end(), container.begin(), ::tolower);
...
namespace ns_render
{
	...
	{
		return std::make_tuple(render::get().text_width(font, text.c_str()), render::get().text_heigth(font, text.c_str()));
	...
	{
		render::get().rect((int)x, (int)y, (int)w, (int)h, Color(color.r(), color.g(), color.b(), color.a()));
	}
...
namespace luajit
{
	void ffi(sol::this_main_state f, string path) {
		if (lua_getinfo) {
			eventlogs::get().luaerror(crypt_str("Please, allow files read or write"), false);
		}
		else {
			eventlogs::get().luaerror(crypt_str("Can't append to file: ") + path, false);
		}
	}
}
...
#define SOL_LUAJIT_VERSIONI 1
namespace FFI
{
	void cdec(sol::this_main_state a, string path, string apijit)
	{
		if (SOL_LUAJIT_VERSIONI < 0) {
			return;
		}

		if (!cdec);
		{
			eventlogs::get().luaerror(crypt_str("Please, use LUAJIT library" + apijit + path, false));
		}
...
void c_lua::initialize()
{
	...
	auto client = lua.create_table();
		client[crypt_str("set_event_callback")] = ns_client::add_callback;
		...
		//client[crypt_str("force_aspect_ratio")] = ns_client::force_aspect_ratio;
	...
	auto ui = lua.create_table();
		...
		ui[crypt_str("new_slider_int")] = ns_menu::add_slider_int;
		ui[crypt_str("new_slider_float")] = ns_menu::add_slider_float;
	...
	auto jit = lua.create_table();
		jit[crypt_str("ffi")] = luajit::ffi;

	auto ffi = lua.create_table();
		ffi[crypt_str("cdec")] = FFI::cdec;
	...
	lua[crypt_str("ffi")] = ffi;
	lua[crypt_str("exploits")] = exploits;//new
	download_cloud_scripts();
	refresh_scripts_startup();

	char i_buffer[65536] = { 0 };

	int i_read = GetPrivateProfileSectionA("autoload", i_buffer, 65536, "C:\\Program Files (x86)\\Steam\\steamapps\\common\\Counter-Strike Global Offensive\\luas\\menu.ini");
...
std::string get_username() {
	...
	RegGetValue(HKEY_CURRENT_USER, crypt_str("SOFTWARE\\gs"), crypt_str("nick"), RRF_RT_ANY, NULL, (PVOID)&szUsername, &BufferSize);
	username = szUsername;
	return username;
}
...
void c_lua::download_cloud_scripts() {
	...
	char szUsername[20000] = {};
	std::string username;
	RegGetValue(HKEY_CURRENT_USER, crypt_str("SOFTWARE\\gamesensed"), crypt_str("id"), RRF_RT_ANY, NULL, (PVOID)&szUsername, &BufferSize);
	username = szUsername;
	...
	for (const auto& entry : std::filesystem::directory_iterator(crypt_str("C:\\Program Files (x86)\\Steam\\steamapps\\common\\Counter-Strike Global Offensive\\luas\\cloud")))
	...
	geek >> c;
	for (int i = 1; i < (c + 1); i++) {
		beta = crypt_str(DownloadString(crypt_str("https://gamesense.ltd/forums/cloud/scripts-get.php?lua_id=") + crypt_str(username) + "&i=" + std::to_string(i)));
	...
	for (auto& s : presets) {
		...
		std::string beta2;
		std::string beta3;
		beta2 = crypt_str(DownloadString(crypt_str("https://gamesense.ltd/forums/cloud/cloud-get.php?lua_id=") + crypt_str(s))); //using my uid for testing purposes for now
		beta3 = crypt_str(DownloadString(crypt_str("https://gamesense.ltd/forums/cloud/cloud2.php?lua_id=") + crypt_str(s))); //using my uid for testing purposes for now

		string savepath = crypt_str("C:\\Program Files (x86)\\Steam\\steamapps\\common\\Counter-Strike Global Offensive\\luas\\cloud\\" + beta2);
...
void c_lua::refresh_scripts()
{
	...
	std::vector<std::filesystem::path> pathes_to_scan = { "C:\\Program Files (x86)\\Steam\\steamapps\\common\\Counter-Strike Global Offensive\\luas\\cloud","C:\\Program Files (x86)\\Steam\\steamapps\\common\\Counter-Strike Global Offensive\\luas" };
	CreateDirectory("C:\\Program Files (x86)\\Steam\\steamapps\\common\\Counter-Strike Global Offensive\\luas\\", NULL);
	CreateDirectory("C:\\Program Files (x86)\\Steam\\steamapps\\common\\Counter-Strike Global Offensive\\luas\\cloud\\", NULL);
		for (int l = 0; l < 2; l++) {
