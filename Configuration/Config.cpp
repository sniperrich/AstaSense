...
void c_config::export_to_clipboard() {
	...
	if (is_number(std::to_string(c_config::get()->i["_preset"])) == false) {
		...
		std::string allFile;
		std::ifstream file(configs_path[c_config::get()->i["_preset"]]);
		copy(std::istream_iterator<char>{ file >> std::noskipws }, {}, back_inserter(allFile));
		file.close();

		std::string data;

		size_t len = strlen(data.c_str()) + 1;
...
void c_config::save() {
	...
	sprintf(file_path, "C:/sb/cfg/%s.ini", (std::string(Config_input_text)).c_str());
...
void c_config::refresh_configs() {
	...
	for (auto& entry : std::filesystem::directory_iterator(("C:\\sb\\cfg"))) {
...
void c_config::load() {
	...
	char file_path[MAX_PATH] = { 0 };
	sprintf(file_path, "C:/sb/cfg/%s", (configs.at(i["_preset"]).c_str()));
...
void c_config::load_defaults() {
	...
	setcolor(c["glowcoll"], 186, 75, 131, 255);
...
void c_config::load_keys() {
	for (int k = 0; k < presets.size(); k++) {
		...
		i[buffer] = GetPrivateProfileIntA("k", buffer, 0, "C:/sb/cfg/csgo_keys.ini");
	}
}
...
void c_config::save_keys() {
	for (int k = 0; k < presets.size(); k++) {
		...
		WritePrivateProfileStringA("k", buffer, value, "C:/sb/cfg/csgo_keys.ini");
	}
}