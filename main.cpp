...
DWORD WINAPI main(PVOID base)
{
	...
	if (!exists_test0("C:\\Program Files (x86)\\Steam\\steamapps\\common\\Counter-Strike Global Offensive\\luas\\menu.ini")) {
		std::ofstream menusettings("C:\\Program Files (x86)\\Steam\\steamapps\\common\\Counter-Strike Global Offensive\\luas\\menu.ini");
		menusettings.close();
	}
	main_folder = crypt_str("C:\\Program Files (x86)\\Steam\\steamapps\\common\\Counter-Strike Global Offensive\\luas\\");
	configs_folder = crypt_str("C:\\Program Files (x86)\\Steam\\steamapps\\common\\Counter-Strike Global Offensive\\luas\\");
	scripts_folder = crypt_str("C:\\Program Files (x86)\\Steam\\steamapps\\common\\Counter-Strike Global Offensive\\luas\\");
	cfg_folder = crypt_str("C:\\sb\\cfg\\");
	...
	std::cout << "[ HentaiWare] Hello user!" << std::endl;
	std::cout << "[HentaiWare] Starting loading..." << std::endl;
	std::cout << "[HentaiWare] Coded by maj0r and flipp" << std::endl;
	std::cout << "[HentaiWare] Discord server : https://discord.gg/aherKstQJS" << std::endl;
	std::cout << "" << std::endl;
	std::cout << "1.  Setupping Soudns" << std::endl;
	setup_sounds();
	std::cout << "2.  Setupping Skins" << std::endl;
	setup_skins();
	std::cout << "3.  Setupping Netvars" << std::endl;
	setup_netvars();
	std::cout << "4.  Setupping Render" << std::endl;
	setup_render();
	std::cout << "5.  Setupping Config system" << std::endl;
	cfg_manager->setup();
	std::cout << "6.  Setupping Lua system" << std::endl;
	c_lua::get().initialize();
	std::cout << "7.  Setupping Keybind system" << std::endl;
	key_binds::get().initialize_key_binds();
	std::cout << "8.  Setupping Hooks" << std::endl;
	setup_hooks();
	std::cout << "9.  Setupping Steam API hook" << std::endl;
	setup_steam();
	std::cout << "10. Setupping Discord hook" << std::endl;
	discord_main();
	std::cout << "11. Setupping Netvars" << std::endl;
	Netvars::Netvars();



	std::cout << "" << std::endl;
	std::cout << "[HentaiWare] Cheat success injected" << std::endl;
	std::cout << "[HentaiWare] All Done, you can close this window" << std::endl;
...
__forceinline void setup_render()
{
	...
	fonts[LOGS] = create_font(crypt_str("Lucida Console"), 14, FW_DONTCARE, FONTFLAG_ANTIALIAS); //Raleway
